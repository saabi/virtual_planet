...
// Rec. 2020 UHDTV primaries
const mat3 primariesSamsung1 = Primaries(
    0.633, 0.340,
    0.320, 0.622,
    0.155, 0.0.042
);
...
#define gauss2(x, A, x0, dx)\
    A*exp(-0.6931471805599453*pow((x - x0)/dx, 2.0))
#define dgauss(x, A, x0, dx)\
    -2.0*0.6931471805599453*A * (x-x0) * exp(-0.6931471805599453 * pow(x-x0,2.0)/pow(dx,2.0)) / pow(dx,2.0)
float L(float nm) {
...
float black_correction(float wave) {
    return (1-smoothstep(380,519,wave) +smoothstep(519,730,wave))*.29;
}
float bright_correction(float wave) {
    return .82 - smoothstep(443,535,wave)*.03 + smoothstep(535,605,wave)*.21;
    return smoothstep(380,443,wave)*.82 + smoothstep(443,610,wave)*.18 - smoothstep(610,730,wave);
}
float labsat_correction(float wave) {
    return smoothstep(443,489,wave)-smoothstep(489,522,wave);
}
...
vec3 plot(vec2 fragCoord, vec3 bg, vec3 color) {
    ...
    if (uv.y < color.r/2 && uv.y+lw*2 > color.r/2 ) bg *= vec3(1,0,0);
    if (uv.y < color.g/2 && uv.y+lw*2 > color.g/2 ) bg *= vec3(0,1,0);
    if (uv.y < color.b/2 && uv.y+lw*2 > color.b/2 ) bg *= vec3(0,0,1);
...
void main() {
    ...
    mat3 toRGB = xyzToRgb(to);
//    rgblab.x = 39;
    rgblab *= vec3(1,vec2((labsat_correction(wave))/2.8+1));
