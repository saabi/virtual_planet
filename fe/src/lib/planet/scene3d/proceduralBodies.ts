...
/**
 * Every tessellated planet/moon in the draw list (mesh tier), ordered largest on-screen
 * first. The selected body is moved to the front when present. Capped at
 * {@link MAX_PROCEDURAL_BODIES} as a GPU/perf safety limit.
 */
export function selectProceduralTargets(
	...
	for (const item of drawList) {
		if (item.lod === 'dot' || !item.screen) continue;
		...
		candidates.push({
			...
			displacementBlend: item.displacementBlend,
			heightBlend: item.heightBlend,
			atmosphereBlend: item.atmosphereBlend,
...
/** Far-to-near order for terrain draws. */
